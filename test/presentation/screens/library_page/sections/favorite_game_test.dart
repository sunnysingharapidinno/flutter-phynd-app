...
import 'favorite_game_test.mocks.dart';
...
void main() {
  ...
  setUp(() {
    ...
    when(mockGameService.getFavoriteGames(
      page: 1,
      limit: 10,
    )).thenAnswer((_) async => (
          data: <FavoriteGame>[],
  ...
  Widget createWidgetUnderTest() {
    return MaterialApp(
      home: Scaffold(
        body: FavoriteGameSection(gameService: mockGameService),
      ),
    );
  }

  group('FavoriteGameSection Widget Tests', () {
    ...
    //   expect(find.text('No Favorited Games'), findsOneWidget);
    //   expect(find.text("You haven't favorited any games yet"), findsOneWidget);
    // });

    testWidgets('displays favorited games when content is available',
        (WidgetTester tester) async {
      final mockGames = [
        FavoriteGame(
          gameSlug: 'game-1',
      ...
      expect(find.text('Favorited Games'), findsOneWidget);
